...
bool calcOdometry(double diff_time)
{
  ...
  //h.new
  float dx, dy, dtheta, dthetb, dalpha, displace;
...
*/
  ...
  dx = 0.02*p*(sinx) + (0.1732*(cosx)-0.01*(sinx))*q - (0.01732*(cosx)+0.01*(sinx))*r ;
  dy =-0.02*p*(cosx) - (0.01732*(sinx)+0.01*(cosx))*q + (-0.01732*(sinx)+0.01*(cosx))*r ;
  dtheta = (-0.0819672)*(p + q + r);
  dthetb = (-1) * dtheta;
  dalpha = (dy) / (dx);
  displace = sqrt( ((double)dx*(double)dx) + ((double)dy*(double)dy) );

  
  odom_pose[0] = odom_pose[0] + displace * cos(dthetb + dalpha); //delta_s * cos(odom_pose[2] + (delta_theta / 2.0));
  odom_pose[1] = odom_pose[1] + displace * sin(dthetb + dalpha); //delta_s * sin(odom_pose[2] + (delta_theta / 2.0));
  odom_pose[2] += atan2(odom_pose[1] , odom_pose[0]); //delta_theta;
